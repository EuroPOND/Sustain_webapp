...
class AbstractSustain(ABC):
    ...
    def run_sustain_algorithm(self):
        ...
        text_file = open(self.output_folder+'/total_likelis.txt', "w")
        
        for s in range(self.N_S_max):
            print('\n ########  Finding ML solution to ',s+1,' cluster problem ###################') 
            pickle_filename_s               = self.output_folder + '/' + self.dataset_name + '_subtype' + str(s) + '.pickle'
            pickle_filepath                 = Path(pickle_filename_s)
            if pickle_filepath.exists():
                print("\n Found pickle file: " + pickle_filename_s + ". Using pickled variables for " + str(s) + " subtype.")
                ...
                text_file.write('\n'+'Overall ML likelihood for '+str(s)+' subtypes problem is '+str(ml_likelihood))
        ...
        fig0.savefig(self.output_folder + '/MCMC_likelihood_iter' + str(self.N_iterations_MCMC) + '.png', bbox_inches='tight')
        fig0.show()
    ...
    def cross_validate_sustain_model(self, test_idxs, select_fold = []):
        text_file2 = open(self.output_folder+'/cross_total_likelis.txt', "w")
        ...
        for fold in range(Nfolds):
            ...
            print('\n'+'Cross-validating fold',fold,'of',Nfolds,'with index',test_idxs[fold])
        ...
        print('\n'+"CVIC across subtype models: " + str(np.mean(CVIC_matrix, 0)))
        text_file2.write('\n'+"CVIC across subtype models: " + str(np.mean(CVIC_matrix, 0)))
        ...
        pylab.savefig(self.output_folder+'/CVIC_Boxplots_'+str(i)+'.png')
        # max like subtype and stage / subject
        N_samples                       = 1000
        ml_subtype,             \
        prob_ml_subtype,        \
    ...
    def combine_cross_validated_sequences(self, N_subtypes, N_folds):
        ...
        assert pickle_filepath.exists(), "\n Failed to find pickle file for full model with " + str(N_subtypes) + " subtypes."
...
#            print('\n ########  Finding ML solution to 1 cluster problem ###################')
            ...
            for ix_cluster_split in range(N_S - 1):
                ...
                if this_N_cluster > 1:
                    ...
                    print('\n Splitting cluster', ix_cluster_split + 1, 'of', N_S - 1)
