...
class ZscoreSustain(AbstractSustain):
    ...
    def _calculate_likelihood_stage(self, sustainData, S):
        ...
        """
        for j in range(N+1):
            x                   = (sustainData.data-np.tile(stage_value[:,j],(M,1)))/sigmat
...
#                print('Iteration', i, 'of', n_iterations, ',', int(float(i) / float(n_iterations) * 100.), '% complete')
    ...
    def _plot_sustain_model(self, samples_sequence, samples_f, n_samples, cval=False, plot_order=None):
        ...
        return fig, ax
    def _plot_sustain_model_D(self, samples_sequence, samples_f, n_samples, cval=False, plot_order=None):

                         
        TITLE_FONT_SIZE                     = 24
        X_ticks_FONT_SIZE                   = 18
        Y_ticks_FONT_SIZE                   = 18
        X_labels_FONT_SIZE                  = 24
        Y_labels_FONT_SIZE                  = 24
        Legend_FONT_SIZE                    = 24 

        colour_mat                          = np.array([[1,1,1],[1, 0, 0], [1, 0, 1], [0, 0, 1]]) #first colour is background, others are zscores
        
        zvalues                         = np.unique(self.stage_zscore)
        N_z                             = len(zvalues)

        colour_tns=np.ones([N_z,3])

        for i in range(N_z):
            colour_tns[i,:]=colour_mat[i+1,:]-colour_mat[i,:]
        ...
        N_S                                 = samples_sequence.shape[0]
        N_bio                               = len(self.biomarker_labels)
        N                                   = samples_sequence.shape[1]        
        
        if N_S == 1:
            fig, ax                         = plt.subplots()
            total_axes                      = 1;
        else:
            fig, ax                         = plt.subplots(N_S,figsize=(15,N*N_S*2.2/N_bio))
            total_axes                      = N_S
            ...
            for i_bio in range(N_bio):
                for i_sta in range(N):
                    ...
                    for i_Z in range(N_z):                      
                        if Z_seq_matrix[i_bio,i_sta] == 0:
                            confus_matrix_c[i_bio,i_sta,0]=colour_mat[0,0]
                            confus_matrix_c[i_bio,i_sta,1]=colour_mat[0,1]
                            confus_matrix_c[i_bio,i_sta,2]=colour_mat[0,2]
                        elif (Z_seq_matrix[i_bio,i_sta] > i_Z) and (Z_seq_matrix[i_bio,i_sta] <= (i_Z+1)):
                            for rgb in range(3):
                                if colour_tns[i_Z,rgb] == 0:
                                    confus_matrix_c[i_bio,i_sta,rgb]=colour_mat[i_Z,rgb]
                                else:
                                    confus_matrix_c[i_bio,i_sta,rgb]=(1/2)-colour_tns[i_Z,rgb]*(((i_Z+1)-Z_seq_matrix[i_bio,i_sta])-(1/2))
                        elif Z_seq_matrix[i_bio,i_sta] > N_z:
                            confus_matrix_c[i_bio,i_sta,0]=colour_mat[i_Z+1,0]
                            confus_matrix_c[i_bio,i_sta,1]=colour_mat[i_Z+1,1]
                            confus_matrix_c[i_bio,i_sta,2]=colour_mat[i_Z+1,2]

            # must be a smarter way of doing this, but subplots(1,1) doesn't produce an array...
            if N_S > 1:
                ax_i                        = ax.flat[i] #ax[i]
                ax_i.imshow(confus_matrix_c, interpolation='nearest') 

                ax_i.set_xticks(np.arange(N))
                ax_i.set_xticklabels(range(1, N+1), fontsize=X_ticks_FONT_SIZE)

                ax_i.set_yticks(np.arange(N_bio))
                ax_i.set_yticklabels(np.array(self.biomarker_labels, dtype='object'), ha='right', fontsize=Y_ticks_FONT_SIZE)
                for tick in ax_i.yaxis.get_major_ticks():
                    tick.label.set_color('black')

                #ax_i.set_ylabel('Biomarker name', fontsize=Y_labels_FONT_SIZE)
                ax_i.set_xlabel('Event position', fontsize=X_labels_FONT_SIZE)
                ax_i.set_title('Group ' + str(i+1), fontsize=TITLE_FONT_SIZE, loc='left')

            else: #**** single plot
                ax.imshow(confus_matrix_c) #, interpolation='nearest')#, cmap=plt.cm.Blues) #[...,::-1]
                ax.set_xticks(np.arange(N))
                ax.set_xticklabels(range(1, N+1), fontsize=X_ticks_FONT_SIZE)

                ax.set_yticks(np.arange(N_bio))
                ax.set_yticklabels(np.array(self.biomarker_labels, dtype='object'), ha='right', fontsize=Y_ticks_FONT_SIZE)

                for tick in ax.yaxis.get_major_ticks():
                    tick.label.set_color('black')

                ax.set_xlabel('Event position', fontsize=X_FONT_SIZE)
                ax.set_title('Group ' + str(i+1), fontsize=TITLE_FONT_SIZE, loc='left')
...
#        plt.tight_layout()
        ...
        legend_labels = []
        for i_Z in range(N_z): 
            linetoappend=Line2D([0], [0], linestyle=None, marker=None, color=colour_mat[i_Z+1,:], lw=4)
            labeltoappend="Z-score = "+str(i_Z+1)
            legend_lines.append(linetoappend)
            legend_labels.append(labeltoappend)    
            print(legend_labels) 
        fig.legend(legend_lines, legend_labels,loc="upper right", ncol=N_z, fontsize=Legend_FONT_SIZE)
    ...
    def subtype_and_stage_individuals_newData(self, data_new, samples_sequence, samples_f, N_samples):
        ...
        #check same number of biomarkers
        numBiom_new                   = data_new.shape[1]         
#        assert numStages_new == self.__sustainData.getNumSamples(), "Number of stages in new data should be same as in training data"
        assert numBiom_new == self.__sustainData.getNumBiomarkers(), "Number of biomarkers in new data should be same as in training data"
        
        numStages_new                   = self.__sustainData.getNumStages()

        sustainData_newData             = ZScoreSustainData(data_new, numStages_new)

        ml_subtype, prob_ml_subtype, ml_stage, prob_ml_stage, prob_subtype                   = self.subtype_and_stage_individuals(sustainData_newData, samples_sequence, samples_f, N_samples)

        return ml_subtype, prob_ml_subtype, ml_stage, prob_ml_stage,prob_subtype
